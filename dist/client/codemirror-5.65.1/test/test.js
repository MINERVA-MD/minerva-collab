...
if (opera_version)
    opera_version = Number(opera_version);
...
test("core_defaults", function () {
    ...
    try {
        ...
        for (var opt in defsCopy)
            defs[opt] = defsCopy[opt];
...
testCM("markTextCSS", function (cm) {
    function present() {
        ...
        for (var i = 0; i < spans.length; i++)
            if (spans[i].style.color && spans[i].textContent == "cdef")
                return true;
    }
...
testCM("markTextWithAttributes", function (cm) {
    function present() {
        var spans = cm.display.lineDiv.getElementsByTagName("span");
        for (var i = 0; i < spans.length; i++)
            if (spans[i].getAttribute("label") == "label" && spans[i].textContent == "cdef")
                return true;
    }
...
testCM("bookmarkCursor", function (cm) {
    var pos01 = cm.cursorCoords(Pos(0, 1)), pos11 = cm.cursorCoords(Pos(1, 1)), pos20 = cm.cursorCoords(Pos(2, 0)), pos30 = cm.cursorCoords(Pos(3, 0)), pos41 = cm.cursorCoords(Pos(4, 1));
    ...
    var new01 = cm.cursorCoords(Pos(0, 1)), new11 = cm.cursorCoords(Pos(1, 1)), new20 = cm.cursorCoords(Pos(2, 0)), new30 = cm.cursorCoords(Pos(3, 0));
...
testCM("selectionPos", function (cm) {
    if (cm.getOption("inputStyle") != "textarea")
        return;
    cm.setSize(100, 100);
    addDoc(cm, 200, 100);
...
testCM("restoreHistory", function (cm) {
    ...
    if (window.JSON)
        storedHist = JSON.parse(JSON.stringify(storedHist));
    eq(storedVal, "abc\nhello");
    cm.setValue("");
    cm.clearHistory();
    eq(cm.historySize().undo, 0);
...
testCM("wrappingAndResizing", function (cm) {
    ...
    forEach([Pos(0, doc.length), Pos(0, doc.length - 1),
        Pos(0, 0), Pos(1, doc.length), Pos(1, doc.length - 1)], function (pos) {
...
testCM("rtlMovement", function (cm) {
    ...
    forEach(["خحج", "خحabcخحج", "abخحخحجcd", "abخde", "abخح2342خ1حج", "خ1ح2خح3حxج",
        ...
        for (var i = 0; i <= line.length; ++i) {
            ...
            if (i == line.length)
                is(cursor.offsetTop > prevY, "next line");
            else
                is(cursor.offsetLeft > prevX, "moved right");
...
testCM("atomicMarker", function (cm) {
    ...
    function atom(ll, cl, lr, cr, li, ri, ls, rs) {
        ...
        if (ls === true || ls === false)
            options.selectLeft = ls;
        if (rs === true || rs === false)
            options.selectRight = rs;
...
testCM("addOverlay", function (cm) {
    cm.addOverlay({
        token: function (stream) {
            ...
            if (!/comment/.test(base.type) && stream.match(/\d+/))
                return "x";
            stream.next();
        }
    });
...
var getChar = function (noExtending) { var res; do {
    res = String.fromCharCode(Math.floor(Math.random() * 0x8ac));
} while ([0x90].indexOf(res.charCodeAt(0)) != -1 || (noExtending && extendingChars.test(res))); return res; };
var getString = function (n) { var res = getChar(true); while (--n > 0)
...
function testMoveBidi(str) {
    testCM("move_bidi_" + str, function (cm) {
        if (cm.getOption("inputStyle") != "textarea" || !cm.getOption("rtlMoveVisually"))
            return;
