...
function eq(a, b, msg) {
    if (a != b)
        throw new Failure(label(a + " != " + b, msg));
}
...
function eqCharPos(a, b, msg) {
    ...
    if (a == b)
        return;
    if (a == null)
        throw new Failure(label("comparing null to " + str(b), msg));
    if (b == null)
...
function eqCursorPos(a, b, msg) {
    eqCharPos(a, b, msg);
    if (a)
        eq(a.sticky, b.sticky, msg ? msg + ' (sticky)' : 'sticky');
}
function is(a, msg) {
    if (!a)
        throw new Failure(label("assertion failed", msg));
}
function countTests() {
    if (!filters.length)
        return tests.length;
...
function parseTestFilter(s) {
    if (/_\*$/.test(s))
        return new RegExp("^" + s.slice(0, s.length - 2), "i");
    else
