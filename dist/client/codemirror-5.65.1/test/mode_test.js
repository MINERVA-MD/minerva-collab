...
(function () {
        ...
        for (var i = 0; i < lines.length; ++i) {
            ...
            if (mode.indent) {
                ...
                if (indent != CodeMirror.Pass && indent != ws.length)
                    (st.indentFailures || (st.indentFailures = [])).push("Indentation of line " + (i + 1) + " is " + indent + " (expected " + ws.length + ")");
            }
    ...
    function highlightOutputsDifferent(o1, o2) {
        ...
        for (var i = 0; i < minLen; ++i)
            if (o1[i].style != o2[i].style || o1[i].text != o2[i].text)
                return i;
        if (o1.length > minLen || o2.length > minLen)
            return minLen;
    }
