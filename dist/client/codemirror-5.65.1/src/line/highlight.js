...
exports.retreatFrontier = exports.takeToken = exports.processLine = exports.getContextBefore = exports.getLineStyles = exports.highlightLine = void 0;
...
class Context {
    ...
    static fromSaved(doc, saved, line) {
        if (saved instanceof SavedContext)
            return new Context(doc, (0, modes_js_1.copyState)(doc.mode, saved.state), line, saved.lookAhead);
        else
            return new Context(doc, (0, modes_js_1.copyState)(doc.mode, saved), line);
    }
    save(copy) {
        let state = copy !== false ? (0, modes_js_1.copyState)(this.doc.mode, this.state) : this.state;
...
function getLineStyles(cm, line, updateFrontier) {
    if (!line.styles || line.styles[0] != cm.state.modeGen) {
        let context = getContextBefore(cm, (0, utils_line_js_1.lineNo)(line));
        let resetState = line.text.length > cm.options.maxHighlightLength && (0, modes_js_1.copyState)(cm.doc.mode, context.state);
...
function getContextBefore(cm, n, precise) {
    ...
    let saved = start > doc.first && (0, utils_line_js_1.getLine)(doc, start - 1).stateAfter;
    let context = saved ? Context.fromSaved(doc, saved, start) : new Context(doc, (0, modes_js_1.startState)(doc.mode), start);
...
function processLine(cm, text, context, startAt) {
    ...
    let stream = new StringStream_js_1.default(text, cm.options.tabSize, context);
...
function callBlankLine(mode, state) {
    ...
    if (!mode.innerMode)
        return;
    let inner = (0, modes_js_1.innerMode)(mode, state);
...
function readToken(mode, stream, state, inner) {
    for (let i = 0; i < 10; i++) {
        if (inner)
            inner[0] = (0, modes_js_1.innerMode)(mode, state).mode;
...
function takeToken(cm, pos, precise, asArray) {
    ...
    pos = (0, pos_js_1.clipPos)(doc, pos);
    let line = (0, utils_line_js_1.getLine)(doc, pos.line), context = getContextBefore(cm, pos.line, precise);
    let stream = new StringStream_js_1.default(line.text, cm.options.tabSize, context), tokens;
    if (asArray)
        tokens = [];
    while ((asArray || stream.pos < pos.ch) && !stream.eol()) {
        ...
        if (asArray)
            tokens.push(new Token(stream, style, (0, modes_js_1.copyState)(doc.mode, context.state)));
    }
...
function runMode(cm, text, mode, context, f, lineClasses, forceToEnd) {
    ...
    let stream = new StringStream_js_1.default(text, cm.options.tabSize, context), style;
...
function findStartLine(cm, n, precise) {
    ...
    for (let search = n; search > lim; --search) {
        if (search <= doc.first)
            return doc.first;
        let line = (0, utils_line_js_1.getLine)(doc, search - 1), after = line.stateAfter;
        if (after && (!precise || search + (after instanceof SavedContext ? after.lookAhead : 0) <= doc.modeFrontier))
            return search;
        let indented = (0, misc_js_1.countColumn)(line.text, null, cm.options.tabSize);
...
function retreatFrontier(doc, n) {
    ...
    for (let line = n - 1; line > start; line--) {
        let saved = (0, utils_line_js_1.getLine)(doc, line).stateAfter;
        // change is on 3
