...
exports.findMaxLine = exports.lineLength = exports.heightAtLine = exports.lineIsHidden = exports.visualLineEndNo = exports.visualLineNo = exports.visualLineContinued = exports.visualLineEnd = exports.visualLine = exports.conflictingCollapsedRange = exports.collapsedSpanAround = exports.collapsedSpanAtEnd = exports.collapsedSpanAtStart = exports.compareCollapsedMarkers = exports.attachMarkedSpans = exports.detachMarkedSpans = exports.removeReadOnlyRanges = exports.stretchSpansOverChange = exports.addMarkedSpan = exports.removeMarkedSpan = exports.getMarkedSpanFor = exports.MarkedSpan = void 0;
const misc_js_1 = require("../util/misc.js");
const pos_js_1 = require("./pos.js");
const saw_special_spans_js_1 = require("./saw_special_spans.js");
...
function stretchSpansOverChange(doc, change) {
    if (change.full)
        return null;
    let oldFirst = (0, utils_line_js_1.isLine)(doc, change.from.line) && (0, utils_line_js_1.getLine)(doc, change.from.line).markedSpans;
    let oldLast = (0, utils_line_js_1.isLine)(doc, change.to.line) && (0, utils_line_js_1.getLine)(doc, change.to.line).markedSpans;
    if (!oldFirst && !oldLast)
        return null;
    let startCh = change.from.ch, endCh = change.to.ch, isInsert = (0, pos_js_1.cmp)(change.from, change.to) == 0;
    // Get the spans that 'stick out' on both sides
    let first = markedSpansBefore(oldFirst, startCh, isInsert);
    let last = markedSpansAfter(oldLast, endCh, isInsert);
    // Next, merge those two ends
    let sameLine = change.text.length == 1, offset = (0, misc_js_1.lst)(change.text).length + (sameLine ? startCh : 0);
    if (first) {
...
function removeReadOnlyRanges(doc, from, to) {
    ...
    doc.iter(from.line, to.line + 1, line => {
        if (line.markedSpans)
            for (let i = 0; i < line.markedSpans.length; ++i) {
                ...
                if (mark.readOnly && (!markers || (0, misc_js_1.indexOf)(markers, mark) == -1))
    ...
    for (let i = 0; i < markers.length; ++i) {
        ...
        for (let j = 0; j < parts.length; ++j) {
            let p = parts[j];
            if ((0, pos_js_1.cmp)(p.to, m.from) < 0 || (0, pos_js_1.cmp)(p.from, m.to) > 0)
                continue;
            let newParts = [j, 1], dfrom = (0, pos_js_1.cmp)(p.from, m.from), dto = (0, pos_js_1.cmp)(p.to, m.to);
...
function compareCollapsedMarkers(a, b) {
    ...
    let fromCmp = (0, pos_js_1.cmp)(aPos.from, bPos.from) || extraLeft(a) - extraLeft(b);
    if (fromCmp)
        return -fromCmp;
    let toCmp = (0, pos_js_1.cmp)(aPos.to, bPos.to) || extraRight(a) - extraRight(b);
    if (toCmp)
        return toCmp;
    return b.id - a.id;
}
...
function collapsedSpanAtSide(line, start) {
    let sps = saw_special_spans_js_1.sawCollapsedSpans && line.markedSpans, found;
    if (sps)
...
function conflictingCollapsedRange(doc, lineNo, from, to, marker) {
    let line = (0, utils_line_js_1.getLine)(doc, lineNo);
    let sps = saw_special_spans_js_1.sawCollapsedSpans && line.markedSpans;
    if (sps)
        for (let i = 0; i < sps.length; ++i) {
            let sp = sps[i];
            ...
            let fromCmp = (0, pos_js_1.cmp)(found.from, from) || extraLeft(sp.marker) - extraLeft(marker);
            let toCmp = (0, pos_js_1.cmp)(found.to, to) || extraRight(sp.marker) - extraRight(marker);
            if (fromCmp >= 0 && toCmp <= 0 || fromCmp <= 0 && toCmp >= 0)
                continue;
            if (fromCmp <= 0 && (sp.marker.inclusiveRight && marker.inclusiveLeft ? (0, pos_js_1.cmp)(found.to, from) >= 0 : (0, pos_js_1.cmp)(found.to, from) > 0) ||
                fromCmp >= 0 && (sp.marker.inclusiveRight && marker.inclusiveLeft ? (0, pos_js_1.cmp)(found.from, to) <= 0 : (0, pos_js_1.cmp)(found.from, to) < 0))
                return true;
        }
}
...
function visualLineNo(doc, lineN) {
    let line = (0, utils_line_js_1.getLine)(doc, lineN), vis = visualLine(line);
    if (line == vis)
        return lineN;
    return (0, utils_line_js_1.lineNo)(vis);
}
...
function visualLineEndNo(doc, lineN) {
    if (lineN > doc.lastLine())
        return lineN;
    let line = (0, utils_line_js_1.getLine)(doc, lineN), merged;
    if (!lineIsHidden(doc, line))
        return lineN;
    while (merged = collapsedSpanAtEnd(line))
        line = merged.find(1, true).line;
    return (0, utils_line_js_1.lineNo)(line) + 1;
}
...
function findMaxLine(cm) {
    ...
    d.maxLine = (0, utils_line_js_1.getLine)(doc, doc.first);
