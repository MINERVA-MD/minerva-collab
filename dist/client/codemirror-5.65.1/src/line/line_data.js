...
exports.buildViewArray = exports.LineView = exports.defaultSpecialCharPlaceholder = exports.buildLineContent = exports.cleanUpLine = exports.updateLine = exports.Line = void 0;
...
const spans_js_1 = require("./spans.js");
...
class Line {
    constructor(text, markedSpans, estimateHeight) {
        this.text = text;
        (0, spans_js_1.attachMarkedSpans)(this, markedSpans);
        this.height = estimateHeight ? estimateHeight(this) : 1;
    }
    lineNo() { return (0, utils_line_js_1.lineNo)(this); }
}
exports.Line = Line;
(0, event_js_1.eventMixin)(Line);
...
function updateLine(line, text, markedSpans, estimateHeight) {
    ...
    if (line.order != null)
        line.order = null;
    (0, spans_js_1.detachMarkedSpans)(line);
    (0, spans_js_1.attachMarkedSpans)(line, markedSpans);
...
function buildLineContent(cm, lineView) {
    ...
    let content = (0, dom_js_1.eltP)("span", null, null, browser_js_1.webkit ? "padding-right: .1px" : null);
    let builder = { pre: (0, dom_js_1.eltP)("pre", [content], "CodeMirror-line"), content: content,
        col: 0, pos: 0, cm: cm,
        trailingSpace: false,
    ...
    for (let i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
        ...
        if ((0, feature_detection_js_1.hasBadBidiRects)(cm.display.measure) && (order = (0, bidi_js_1.getOrder)(line, cm.doc.direction)))
            builder.addToken = buildTokenBadBidi(builder.addToken, order);
        builder.map = [];
        let allowFrontierUpdate = lineView != cm.display.externalMeasured && (0, utils_line_js_1.lineNo)(line);
        insertLineContent(line, builder, (0, highlight_js_1.getLineStyles)(cm, line, allowFrontierUpdate));
        if (line.styleClasses) {
            if (line.styleClasses.bgClass)
                builder.bgClass = (0, dom_js_1.joinClasses)(line.styleClasses.bgClass, builder.bgClass || "");
            if (line.styleClasses.textClass)
                builder.textClass = (0, dom_js_1.joinClasses)(line.styleClasses.textClass, builder.textClass || "");
        }
        // Ensure at least a single node is present, for measuring.
        if (builder.map.length == 0)
            builder.map.push(0, 0, builder.content.appendChild((0, feature_detection_js_1.zeroWidthElement)(cm.display.measure)));
    ...
    (0, event_js_1.signal)(cm, "renderLine", cm, lineView.line, builder.pre);
    if (builder.pre.className)
        builder.textClass = (0, dom_js_1.joinClasses)(builder.pre.className, builder.textClass || "");
    return builder;
}
exports.buildLineContent = buildLineContent;
function defaultSpecialCharPlaceholder(ch) {
    let token = (0, dom_js_1.elt)("span", "\u2022", "cm-invalidchar");
...
function buildToken(builder, text, style, startStyle, endStyle, css, attributes) {
    ...
    if (!special.test(text)) {
        ...
        while (true) {
            ...
            if (skipped) {
                ...
                if (browser_js_1.ie && browser_js_1.ie_version < 9)
                    content.appendChild((0, dom_js_1.elt)("span", [txt]));
                else
            ...
            if (m[0] == "\t") {
                ...
                txt = content.appendChild((0, dom_js_1.elt)("span", (0, misc_js_1.spaceStr)(tabWidth), "cm-tab"));
                txt.setAttribute("role", "presentation");
                txt.setAttribute("cm-text", "\t");
                builder.col += tabWidth;
            }
            else if (m[0] == "\r" || m[0] == "\n") {
                txt = content.appendChild((0, dom_js_1.elt)("span", m[0] == "\r" ? "\u240d" : "\u2424", "cm-invalidchar"));
    ...
    if (style || startStyle || endStyle || mustWrap || css || attributes) {
        ...
        if (endStyle)
            fullStyle += endStyle;
        let token = (0, dom_js_1.elt)("span", [content], fullStyle, css);
        if (attributes) {
        ...
        if (nextChange == pos) { // Update current marker set
            ...
            for (let j = 0; j < spans.length; ++j) {
                ...
                if (m.type == "bookmark" && sp.from == pos && m.widgetNode) {
                    ...
                    if (m.collapsed && (!collapsed || (0, spans_js_1.compareCollapsedMarkers)(collapsed.marker, m) < 0))
                        collapsed = sp;
                }
...
function LineView(doc, line, lineN) {
    ...
    this.rest = (0, spans_js_1.visualLineContinued)(line);
    // Number of logical lines in this visual line
    this.size = this.rest ? (0, utils_line_js_1.lineNo)((0, misc_js_1.lst)(this.rest)) - lineN + 1 : 1;
    this.node = this.text = null;
    this.hidden = (0, spans_js_1.lineIsHidden)(doc, line);
}
...
function buildViewArray(cm, from, to) {
    ...
    for (let pos = from; pos < to; pos = nextPos) {
        let view = new LineView(cm.doc, (0, utils_line_js_1.getLine)(cm.doc, pos), pos);
