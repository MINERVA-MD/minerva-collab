...
exports.setScrollLeft = exports.setScrollTop = exports.updateScrollTop = exports.scrollToCoordsRange = exports.scrollToRange = exports.scrollToCoords = exports.ensureCursorVisible = exports.addToScrollTop = exports.scrollIntoView = exports.scrollPosIntoView = exports.maybeScrollWindow = void 0;
...
function maybeScrollWindow(cm, rect) {
    if ((0, event_js_1.signalDOMEvent)(cm, "scrollCursorIntoView"))
        return;
    ...
    if (rect.top + box.top < 0)
        ...
        doScroll = false;
    if (doScroll != null && !browser_js_1.phantom) {
        let scrollNode = (0, dom_js_1.elt)("div", "\u200b", null, `position: absolute;
                         top: ${rect.top - display.viewOffset - (0, position_measurement_js_1.paddingTop)(cm.display)}px;
                         height: ${rect.bottom - rect.top + (0, position_measurement_js_1.scrollGap)(cm) + display.barHeight}px;
...
function scrollPosIntoView(cm, pos, end, margin) {
    ...
    if (!cm.options.lineWrapping && pos == end) {
        ...
        end = pos.sticky == "before" ? (0, pos_js_1.Pos)(pos.line, pos.ch + 1, "before") : pos;
        pos = pos.ch ? (0, pos_js_1.Pos)(pos.line, pos.sticky == "before" ? pos.ch - 1 : pos.ch, "after") : pos;
    }
    for (let limit = 0; limit < 5; limit++) {
        let changed = false;
        let coords = (0, position_measurement_js_1.cursorCoords)(cm, pos);
        let endCoords = !end || end == pos ? coords : (0, position_measurement_js_1.cursorCoords)(cm, end);
...
function calculateScrollPos(cm, rect) {
    let display = cm.display, snapMargin = (0, position_measurement_js_1.textHeight)(cm.display);
    if (rect.top < 0)
        rect.top = 0;
    let screentop = cm.curOp && cm.curOp.scrollTop != null ? cm.curOp.scrollTop : display.scroller.scrollTop;
    let screen = (0, position_measurement_js_1.displayHeight)(cm), result = {};
    if (rect.bottom - rect.top > screen)
        rect.bottom = rect.top + screen;
    let docBottom = cm.doc.height + (0, position_measurement_js_1.paddingVert)(display);
    ...
    let screenw = (0, position_measurement_js_1.displayWidth)(cm) - display.gutters.offsetWidth;
...
function resolveScrollToPos(cm) {
    ...
    if (range) {
        ...
        let from = (0, position_measurement_js_1.estimateCoords)(cm, range.from), to = (0, position_measurement_js_1.estimateCoords)(cm, range.to);
...
function updateScrollTop(cm, val) {
    if (Math.abs(cm.doc.scrollTop - val) < 2)
        return;
    if (!browser_js_1.gecko)
        (0, update_display_js_1.updateDisplaySimple)(cm, { top: val });
    setScrollTop(cm, val, true);
    if (browser_js_1.gecko)
        (0, update_display_js_1.updateDisplaySimple)(cm);
